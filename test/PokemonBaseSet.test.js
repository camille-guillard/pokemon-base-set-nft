...
let contract, contractAddress, owner, addr1, addr2, addr3, now, subscriptionId, vrfCoordinatorAddress, keyHash, VRFCoordinatorV2_5Mock;
...
describe("Pokémon Base Set - Setters", function () {
  ...
  it("Should setting the callbackGasLimit by the owner", async function () {
    expect(await contract.connect(owner).callbackGasLimit()).to.equal(4000000);
    await contract.connect(owner).setCallbackGasLimit(0);
    expect(await contract.connect(owner).callbackGasLimit()).to.equal(0);
  });
  ...
  it("Should setting the requestConfirmations by the owner", async function () {
    expect(await contract.connect(owner).requestConfirmations()).to.equal(3);
    await contract.connect(owner).setRequestConfirmations(2);
    expect(await contract.connect(owner).requestConfirmations()).to.equal(2);
  });
